...
df <- data.frame(x = c(4.25, 1, 0.5, 2.5), y = c(2.75, 1, 5.5, 0.5), 
...
ggplot() +
  geom_ellipse(aes(x0 = 4.25, y0 = 2.75, a = 5, b = 1.5, angle = 0.5), alpha = 0.75, fill = "#d73027") + ## assoc
  ...
  geom_ellipse(aes(x0 = 2.5, y0 = 0.5, a = 3, b = 0.25, angle = 0), alpha = 0.75, fill = "#4575b4") + ## scent
  geom_text(data = df, aes(x, y, label = family)) +
  add_phylopic(img, 1, y = 9, x = 9, ysize = 2) +
  ylab("") +
  xlab("") +
  scale_x_continuous(limit = c(-1, 10), breaks=c(0, 2.5, 5, 7.5, 10), 
                     labels= c("1 Second", "1 Minute",
                             "1 Hour", "1 Day", "1 Month")) +
  scale_y_continuous(limit = c(-1, 10), breaks=c(0, 2.5, 5, 7.5, 10), 
                     labels= c("1m", "10m",
                               "100m", "1000m", "10000m")) +
  coord_fixed() +
  theme(
