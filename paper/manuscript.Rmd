...
[@Integrated_2013]<!--TODO: check ref-->. Available land cover classification included nine habitat
types at 30 m resolution. We reclassified the land cover types into three
categories using the R
package `raster` [@Hijmans_2019]: foraging, open, and forest habitats. Foraging habitat consisted of lichen,
open habitat consisted of wetland, rocky barrens, and anthropogenic, while forest habitat consisted of coniferous forest, conifer scrub,
...
<!-- TODO: better lead in -->
Within each layer, we generated proximity-based social networks using the R package `spatsoc` [@Robitaille_2019] in `r version$version.string` [@R_Core_Team_2019]. We 
assumed individuals were associating if simultaneous GPS
fixes, i.e., recorded within 5 minutes of each other, were within the spatial distance 
threshold of one another. Typically for ungulates and other gregarious
mammals, the ‘chain rule’ is applied for group assignment. <!-- TODO: simplify here-->For GPS data, the
...
separated by more than the given distance [@Farine_2015]. <!-- TODO: rm repeat?-->Social groups were
...
SRI, in each layer-specific season and land cover context. Extending degree
...
calculated the similarity of layers between seasons, across land cover classes
...
We modified the distance threshold, hereafter the social <!--distance-->threshold, required for
...
caribou. <!-- TODO: clarify not attempting ML with multiple behaviours -->

### Spatial scale

To assess the influence of spatial scale on multilayer networks, we aggregated
our land cover raster at varying scales relevant to caribou ecology. We
...
Increasing land cover resolution resulted in a decreased availability of forage,
...
forest and open habitats but varied in winter forage varied across land cover
...
summer and winter forage because, at all land cover resolutions greater than 30
...
land cover resolution, at a much lower rate. Connective redundancy was moderately
high but decreased slightly as land cover resolution increased, indicating that
many neighbours were the same between layers, but the proportion of neighbours
in each layer slightly decreased at increasing land cover resolutions (Figure
\@ref(fig:lcres)d). Overall, these results indicate the importance of matching
land cover resolution to scale of selection as well as ensuring the land cover
...
knitr::include_graphics('../graphics/figure-1.png')
```
...
knitr::include_graphics('../graphics/figure-3.png')
```
...
knitr::include_graphics('../graphics/figure-4.png')
```

\newpage

```{r socres, fig.cap='(ref:socres)'}
knitr::include_graphics('../graphics/figure-5.png')
```

\newpage

```{r nobs, fig.cap='(ref:nobs)'}
knitr::include_graphics('../graphics/figure-6.png')
```
