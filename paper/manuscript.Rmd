...
(habitat type), and temporal (seasonal) scales.--> Caribou are gregarious
...
decrease in rate after 100 m. These results suggest the optimal social scale at
which groups should be assigned is likely somewhere between ~20 - 100 m based on
the rate at which network metrics stabilized. For ungulates, groups are often
...
increased caribou sociality. Edge overlap was higher in time windows 1, 6-11 and
...
@Holekamp_2011]. Defining the social scale at which to generate networks is an
...
(*Rangifer tarandus*) sociality differed between seasons, where social
associations were stronger in winter and contrastingly low in summer when
...
open and forest habitats (Figure \@ref(fig:socres) D). Forage is a rare and
patchy habitat on Fogo Island, as is indicated by a decrease in the proportion
of relocations in this habitat type with increasing spatial resolution (Figure
S1)<!--TODO-->. Similar to increasing spatial distance threshold, the influence
...
increasing spatial resolution and variation between individuals. Forage and open
layers had greater individual variation in graph strength, while forage was the
sole layer to show large responses in graph strength to increasing spatial
...
differences in sociality in discrete time windows. Using an ordinal multilayer
network consisting of 20 discrete time windows throughout the study period, we
observed periods of increased sociality in caribou on Fogo Island (Figure
\@ref(fig:temp)). Edge overlap was higher between approximately October and May
(time windows 1, 6-11, 15-20) compared to between approximately May and October
(time windows 2-5, 12-15) (Figure \@ref(fig:temp) A).
<!-- not done -->

Furthermore, the selection of time windows is essential for both capturing life
stages of interest and having sufficient data to construct robust networks.
Social phenotypes can change over ontogeny in ways that can affect fitness
[@Berger_2015; @Brent_2017]. Further complicating the issue, individuals can
vary in their rate of development [@Tarka_2018]. This variation can make
defining the appropriate temporal scale for testing the fitness implications of
an individual’s social phenotype over different stages of ontogeny difficult.
However, considering fine temporal scales has been show to provide insight across developmental
stages that may otherwise be missed [@Turner_2017]. Furthermore, given the non-independent nature of social interactions and associations,
especially over time, it can be challenging to test hypotheses about social dynamics with a
suitable null control [@Farine_2017; @Proskurnikov_2017]. Using the framework of
multilayer networks, ordinal layers of discrete time windows provide a way
to make appropriate null models to test questions about the dynamics of an
individual’s social network position over multiple aggregated time periods. <!--hmm-->
...
Our case study integrates variation in temporal and spatial scale
