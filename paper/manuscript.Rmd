...
the the role of individuals and the similarity and relevance of individual layers. Degree
centrality, the number of direct connections an individual has to other individuals in the
network, is the basis of many calculations. We calculated multidegree, the sum of degree
centrality across layers, and neighbourhood, the sum of unique neighbours to each
individual[@Berlingerio_2012; @Kivela_2014]. Comparing the multidegree and number of
...
strength, degree weighted by the values of the tie, for each individual for each 
layer-specific season and landcover context <!-- citation?-->. Finally, we calculated the 
similarity of layers between seasons, across landcover classes (e.g.: winter open compared 
to summer open) using the Pearson correlation coefficient [@Br_dka_2018].
