...
Multiscale ecology has been integrated into, for example, species distribution modelling
[@Elith_2009], habitat selection [@Mayor_2007], and food webs [@Sugihara_1989].
Meanwhile, in the context of animal behaviour, certain behaviours
...
Sociality exists across both temporal and spatial scales [@Whitehead_2008]. For
...
temporal scales [e.g. @Piza_Roca_2018]. Although the social environment
...
@Farine_2015a]. We therefore define social scale as the explicit definition 
...
@Wey_2008; @Croft_2011]. It provides insight into the
...
sociality in a monolayer network, drastically simplifying the
...
[@Pilosof_2017]. Multilayer networks are composed of multiple layers, each
...
networks, and neural networks [@Silk_2018]. Scale within multilayer
...
We examined scale-dependent multilayer networks motivated by behavioural, landscape, 
and spatial ecology to and applied this framework to caribou (*Rangifer
tarandus*) on Fogo Island, Newfoundland, Canada. We use study to
...
and temporal scale improve our understanding of the relationship between 
social association and seasonal resource availability? We conclude with further 
development and application of scale in multilayer
networks with respect to habitat selection and space use, 
movement ecology and collective movement, and phenotypes and individual fitness.
...
In winter, caribou dig holes in the snow, termed craters, to access forage
...
`r DT[, format(min(datetime), '%F')]` and `r DT[, format(max(datetime), '%F')]`. 
...
between ~20 - 100 m based on the rate at which variance in graph strength decreased. 
GPS data allows [e.g. from this study and @Peignier_2019; @Lesmerises_2018] us to
...
decreasing in foraging habitat (14.5% at 30 m - <2% at 1000 m).  Overall, these
results indicate the importance of matching land cover resolution to scale of
selection as well as ensuring the land cover product used has a resolution fine
...
15-20 (between `r timedefs[timecut == 1, paste(mindate, maxdate, sep = ' and ')]`,
...
than in time windows 2-5 and 12-15 (between 
`r timedefs[timecut %in% 2:5, paste(min(mindate), max(maxdate), sep = ' and ')]`
...
series between  
`r timedefs[timecut %in% 9:10, paste(min(mindate), max(maxdate), sep = ' and ')]`. 
...
decreasing in variance after a certain number of observations are included, the results
